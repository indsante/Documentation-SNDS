# Localiser les bénéficiaires dans le SNDS 
<!-- SPDX-License-Identifier: MPL-2.0 -->
La localisation du bénéficiaire dans le SNDS est possible grâce aux variables indiquant le **département et la commune de résidence**. A l’heure actuelle, il n’existe pas de niveau plus fin de localisation. Ces variables sont mises à disposition dans différentes tables du SNDS, dans des formats différents et parfois avec des informations contradictoires ou à corriger.
...
En guise de préambule, plusieurs remarques importantes :
* Selon les univers, le code commune ne correspond pas à la même information. Dans le *SNIIRAM* et les *causes de décès*, il s’agit du **code Insee**, dans le *PMSI* il s’agit du **code géographique**, un code postal parfois agrégé lorsque les communes sont trop petites. Dans le SNIIRAM, il est remonté via la carte vitale au moment du soin ou via les bases de données des caisses de sécurité sociale. Dans le PMSI il est déclaré par le bénéficiaire lors du séjour. Dans les causes de décès, il s’agit de l’information sur le certificat de décès.
* Il existe différents critères de décision pour associer un bénéficiaire à un lieu de résidence. La CNAM retient souvent le **dernier département de résidence**, mais il est possible d’utiliser d’autres critères tels que le département associé au plus grand nombre de soins sur la période d’étude, le département de l’organisme de remboursement du bénéficiaire, ou encore le département du professionnel exerçant le soin.  

Concernant la **qualité de l’information**, on peut considérer que :
* Au niveau des départements, la plupart des codes sont bons, sauf pour la Corse, les DOM, et les bénéficiaires de certaines sections locales mutualistes étudiantes (SLM 617)
* Au niveau communal, beaucoup de corrections à faire en recourant notamment à des tables de correction a priori non exhaustives.

## Les variables utiles : département et commune 
Où les trouver ?
...
| **PMSI MCO/SSR/HAD** | **Table séjour B** [T_MCOAA_B](../tables/PMSI/PMSI MCO/TMCOaannB.md)                | BDI_DEP                       | BDI_COD (str(5))          |                                                                                                                    |
| **Carto des pathos** | **Table individus** [CT_IND_AAAA_GN](../tables/CARTOGRAPHIE_PATHOLOGIES/CR_IND_AAAA_GN.md)          | dpt                           | Non disponible                         | Code reconstitué à partir des informations du DCIR et du PMSI et corrigé si besoin                                 |
| **Causes de décès**  | **Table cause initiale de décès** [KI_CCI_R](../tables/Causes de décès/KI_CCI_R.md)  | BEN_RES_DPT                   | BEN_RES_COM               | Les codes ont été transformés pour coller aux référentiels du SNIIRAM IR_DPT_V et IR_GEO_V                         |



## Construction de la localisation dans le SNIIRAM

Si on souhaite travailler *à un niveau départemental*, la variable du département peut s’utiliser seule. Mais dans certains cas, la variable commune est utile pour bien reconstituer le code du département (notamment pour distinguer les DOM entre eux, ainsi que les deux départements de Corse).


Si on souhaite travailler *au niveau communal* dans le SNIIRAM, il faut utiliser l’information des **deux variables**, département et commune.  
...
*  Les DOM :    
    * Pour les bénéficiaires du RG, le code département est ‘097’ et la règle dominante fonctionne
    * Pour la MSA et le RSI, le code département est entre 971 et 976, 
...
Exemple de code pour reconstruire la variable *depcom* à 5 positions à partir des variables BEN_RES_DPT et BEN_RES_COM
...
* Un certain nombre de codes communes et de codes départements sont manquants et codés en 000,099,999. Pour améliorer l’information, il est possible d’utiliser le département de l’organisme d’affiliation du bénéficiaire *(substr(org_aff_ben,4,3))*.
...
* Certains codes communes Insee sont erronés pour différentes raisons et peuvent être corrigés (de préférence dans l’ordre qui suit):
    * La nomenclature des codes communes évolue régulièrement et les caisses ne répercutent pas nécessairement toutes les évolutions de codage. Elles utilisent donc parfois des codes Insee qui n’existent plus.  
...
La méthode la plus rigoureuse pour tenir compte des spécificités du codage de la variable département dans le SNIIRAM (Corse, DOM, etc.) est de repartir de la variable du code commune à 5 positions créée d’après le code ci-dessus, et d’en extraire le département à l’aide du code suivant :
```sas
...
Si on souhaite travailler *à un niveau communal*, la variable commune BDI_COD s’utilise également seule dans le PMSI (5 positions). Elle donne le code géographique du lieu de résidence déclaré par le patient, qui correspond au code postal ou à un regroupement lorsque les communes sont de moins de 1000 habitants.
 
Différences par rapport au SNIIRAM :
* La Corse : codée 2A ou 2B
* Les DOM : codés 9A, 9B, 9C, 9D et 9F

Une table **PMSI_CORRESP** est mise à disposition par l’ATIH pour passer des codes géographiques aux codes postaux. Cette table n’est cependant pas bijective et génère des doublons lorsque les codes géographiques sont associés à plusieurs codes postaux.
