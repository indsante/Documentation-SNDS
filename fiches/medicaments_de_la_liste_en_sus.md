...
Ils sont regroupés sous le code prestation 3320 dans la table prestation <PreviewPage text="ER_PRS_F" link="../tables/ER_PRS_F/" />. Le détail des remboursements par code UCD se trouve dans la table affinée <PreviewPage text="ER_UCD_F" link="../tables/ER_UCD_F/" /> (variable `UCD_UCD_COD`) en posant le filtre `UCD_TOP_UCD`=1 (la modalité 0 correspond aux rétrocessions). On trouve dans <PreviewPage text="ER_UCD_F" link="../tables/ER_UCD_F/" /> les variables `UCD_TTF_MNT` (le montant TTC facturé) et `UCD_DLV_NBR` (le nombre d'unités délivrées). Dans le DCIRS, il faut considérer la table NS_UCD_F, dans laquelle on retrouve les mêmes variables que dans le DCIR.  
