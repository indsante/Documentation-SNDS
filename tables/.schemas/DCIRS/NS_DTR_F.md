...
`TRS_PRS_IDE`|
integer|Code affiné du transport (forfait, km, majoration, supplément)||
`TRS_ACT_PRU`|
number|Prix unitaire du code affiné du transport||
`TRS_TRS_MTF`|
integer|Motif du transport||
`TRS_ACT_QSN`|
integer|Nombre de Km facturés ou de forfaits ou de majorations ou de suppléments||
